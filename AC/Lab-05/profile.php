<?php
/**
 * VULNERABLE PROFILE PAGE - User ID Controlled by Request Parameter (IDOR)
 * 
 * VULNERABILITY: This page uses $_GET['id'] to fetch user data without
 * verifying that the logged-in user is authorized to view that profile.
 * 
 * EXPLOIT: Simply change the 'id' parameter in the URL to view any user's profile.
 * Example: profile.php?id=carlos
 */
...
// VULNERABLE CODE: Uses URL parameter directly without authorization check
// The application trusts the 'id' parameter from the URL
$requestedUser = $_GET['id'] ?? $_SESSION['username'];

$pdo = getDBConnection();

// VULNERABILITY: Fetches ANY user's data based on the URL parameter
...
<head>
    ...
    <title><?php echo $user ? htmlspecialchars($user['full_name']) : 'User'; ?> - IDORLab</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        ...
        <?php else: ?>
            <?php if ($requestedUser !== $_SESSION['username']): ?>
