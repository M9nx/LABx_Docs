<?php
/**
 * Authentication Category - Progress Helper
 * Provides lab progress tracking functions for all Auth labs
 */
...
function isLabSolved($labNumber) {
    ...
    $conn = @new mysqli($creds['host'], $creds['user'], $creds['pass'], 'auth_progress');
...
function markLabSolved($labNumber) {
    ...
    if ($conn->connect_error) {
        ...
        $conn->query("CREATE DATABASE IF NOT EXISTS auth_progress");
        $conn->select_db('auth_progress');
